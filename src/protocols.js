...
export const swimRandomly = {
	swim: function() {
		...
		const deltaX = (this.constructor.name === 'Shark') ? -2 : -1
...
export const eatAllNearbyFish = {
	feed: function() {
		...
		if (nearbyFish.length > 0) {
			console.log(`A ${this.emoji} has eaten ${nearbyFish.length} ${nearbyFish[0].emoji}!`);
...
export const eatSharksIfTooMany = {
	feed: function() {
		...
		if (nearbyAgents.length > 0) {
			console.log(`A ${this.emoji} has eaten ${nearbyAgents.length} ${nearbyAgents[0].emoji}!`);
